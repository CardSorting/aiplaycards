...
interface DuelListing {
  ...
  createdAt: string;
  // YuGiOh card specific fields
  name: string;
  cardType: string; // Monster, Spell, Trap
  cardSubtype?: string;
  cardRare?: string; // N, R, UR
  cardAttr?: string; // DIVINE, EARTH, WATER, etc.
  cardRace?: string;
  cardLevel?: string;
  cardATK?: string;
  cardDEF?: string;
...
export default function DuelMarketplacePage() {
  ...
  // Card listings state
  const [listings, setListings] = useState<DuelListing[]>([]);
  ...
  // Duel-specific filters
  const cardTypes = ['Monster', 'Spell', 'Trap'];
  const cardRarities = ['N', 'R', 'SR', 'UR'];
  const sortOptions = [
  ...
  const filteredListings = (items: DuelListing[]) => {
    let filtered = items;
    if (selectedTypes.length > 0) {
      filtered = filtered.filter(i => selectedTypes.includes(i.cardType));
    }
    if (selectedRarities.length > 0) {
      filtered = filtered.filter(
        i => i.cardRare && selectedRarities.includes(i.cardRare),
      );
    }
    return filtered;
  };
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const duelListings: DuelListing[] = Array.isArray(data?.data)
          ? data.data
          : [];
        setTotal(Number(data?.total || 0));
        setListings(duelListings);
      } catch (error) {
        console.error('Error fetching duel marketplace data:', error);
    ...
    >
      ...
      <Stack spacing={{ xs: 1.5, sm: 2 }} sx={{ mb: { xs: 3, md: 4 } }}>
        <Breadcrumbs aria-label="breadcrumb">
          ...
          <Typography color="text.primary">Duel Cards</Typography>
        </Breadcrumbs>
        ...
        >
          <Box>
            ...
            >
              Discover and collect unique duel cards from the community
            </Typography>
          </Box>
                ...
                >
                  <Skeleton
                    ...
                    sx={{
                      ...
                      height: { xs: 180, sm: 270, md: 360 }, // Different aspect ratio for YuGiOh
                    }}
                  />
                </Box>
                ...
                >
                  ...
                  <Box
                    sx={{
                      ...
                      minHeight: {
                        xs: 180,
                        sm: viewMode === 'list' && !isMobile ? 140 : 240,
                        md: viewMode === 'list' ? 140 : 320,
                      },
                  ...
                  >
                    <Box
                      ...
                      sx={{
                        ...
                        width: {
                          xs: 100,
                          sm: viewMode === 'list' && !isMobile ? 120 : 160,
                    ...
                    >
                      <YugiohCardDisplayWrapper
                        cardData={{
                          ...
                          cardType: listing.cardType as any,
                          cardSubtype: listing.cardSubtype || '',
                          cardRare: listing.cardRare || 'N',
                          cardAttr: listing.cardAttr as any,
