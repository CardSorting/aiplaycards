...
interface SpecialListing {
  ...
  type: string; // normalized to rarity for display
  supertype: string;
  subtype?: string | null;
}

export default function SpecialMarketplacePage() {
  ...
  // Card listings state
  const [listings, setListings] = useState<SpecialListing[]>([]);
  ...
  // Special collection specific filters
  const cardRarities = ['common', 'uncommon', 'rare', 'epic', 'legendary'];
  const sortOptions = [
  ...
  const filteredListings = (items: SpecialListing[]) => {
    let filtered = items;
    ...
    if (selectedCategories.length > 0) {
      filtered = filtered.filter(
        i => i.categoryName && selectedCategories.includes(i.categoryName),
      );
    }
  ...
  // Get unique categories from current listings
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        console.error('Error fetching special marketplace data:', error);
    ...
    >
      ...
      <Stack spacing={{ xs: 1.5, sm: 2 }} sx={{ mb: { xs: 3, md: 4 } }}>
        <Breadcrumbs aria-label="breadcrumb">
          ...
          <Typography color="text.primary">Special Collection</Typography>
        </Breadcrumbs>
        ...
        >
          <Box>
            <Typography
              ...
              sx={{ mb: 1 }}
            >
              Special Collection Marketplace
            </Typography>
            <Typography
              variant="body1"
            ...
            >
              Discover and collect exclusive special cards from the community
            </Typography>
          </Box>
          ...
          >
            ...
            >
              <TextField
                ...
                placeholder="Search special cards..."
                value={search}
            ...
            <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
              {cardRarities.map(rarity => (
                <Chip
                  ...
                  variant={
                    selectedRarities.includes(rarity) ? 'filled' : 'outlined'
                  }
                  clickable
                  onClick={() => {
                    ...
                    <Stack spacing={{ xs: 1, sm: 1.5 }} sx={{ height: '100%' }}>
                      ...
                      >
                        <Chip
                          ...
                          sx={{
                            backgroundColor: getRarityColor(listing.rarity),
                            color: 'white',
                            fontWeight: 'bold',
