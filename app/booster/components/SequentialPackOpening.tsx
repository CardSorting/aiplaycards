...
import { CardDisplayWrapper } from '../../../src/components/CardDisplayWrapper';
import { normalizeCardData } from '../../../src/components/CardDisplayWrapper/utils';
import { CardData } from '../../../src/components/CardDisplayWrapper/types';
import { useBoosterOpening } from '../hooks/useBoosterOpening';
import { getPackBySlug } from '../../../src/features/booster/packs';
import {
  cardImgHeight,
  cardImgWidth,
} from '../../../src/features/cardEditor/cardStyles';
...
export default function SequentialPackOpening({
  ...
  const getCardDisplayData = (card: any): CardData => {
    ...
    if (card.createdCardId) {
      // Return a promise-based approach - the component will handle the async loading
      return {
        ...
        moves:
          card.card?.move1 || card.card?.move2
            ? [card.card.move1, card.card.move2].filter(Boolean).map(move => ({
                name: move!.name,
