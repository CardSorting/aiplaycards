...
_Updated pricing: Booster packs now cost only 38 credits (38¢) each! 🎉_

### [Use the tool on https://playmoretcg.com](https://playmoretcg.com)
...
### Booster Queue Worker

The booster pack opening queue worker starts automatically with the Next.js app. For more control:

- To run worker separately: `npm run worker:booster`
- To disable auto-start: Set `DISABLE_BOOSTER_WORKER=true` in environment
- To check worker status: Visit `/api/worker-status`
- See [Worker Documentation](./docs/WORKER_STARTUP.md) for details
...
1. **Update API Routes** - Add social endpoints to collections API
2. **Build UI Components** - Likes, comments, ratings, followers components
3. **Implement Search** - Tag-based discovery interface
