...
export const CommunityFeed: FC<{
  title: string;
  items?: FeedItem[];
  empty?: ReactNode;
  loading?: boolean;
}> = ({ title, items = [], empty, loading = false }) => {
          ...
          >
            {Array.from({ length: 6 }, (_, i) => (
              <Grid item xs={12} sm={6} md={4} key={`skeleton-${i}`}>
                <Card
                  sx={{
                    height: 280, // Fixed height to prevent layout shift
                    display: 'flex',
                    flexDirection: 'column',
                  }}
                >
              ...
              >
                ...
                <Stack spacing={1} textAlign="center">
                  ...
                  <Typography color="text.secondary">
                    Be the first to share your Trading card creations with the
                    community
                  </Typography>
                </Stack>
                <Stack
              ...
              {items.map(it => (
                <Grid item xs={12} sm={6} md={4} key={it.id}>
                  <Card
                    ...
                    sx={{
                      textDecoration: 'none',
                      height: 280, // Increased height to accommodate background image
                      display: 'flex',
                      flexDirection: 'column',
                      cursor: 'pointer',
                      position: 'relative',
                      ...
                      >
                        <Box
                          sx={{
                            ...
                            fontSize: '12px',
                            fontWeight: 'bold',
                            color: it.backgroundImage
                              ? 'primary.main'
                              : 'text.secondary',
                          }}
                          role="img"
                          aria-label={`${it.creator.handle}'s avatar`}
                        >
                          {it.creator.avatarUrl ? (
                            <Image
                              src={it.creator.avatarUrl}
                              alt={`${it.creator.handle}'s avatar`}
                              width={28}
                              height={28}
                            />
                          ) : (
                            it.creator.handle.charAt(0).toUpperCase()
                          )}
                        </Box>
                        <Typography
                          variant="body2"
                          sx={{
                            color: it.backgroundImage
                              ? 'rgba(255,255,255,0.9)'
                              : 'text.secondary',
                            fontWeight: it.backgroundImage ? 600 : 400,
                          }}
                        >
                          @{it.creator.handle}
                        </Typography>
                        {it.allowlisted === false && (
                          <Chip
                      ...
                      {it.metrics && (
                        <Stack direction="row" spacing={1} sx={{ mt: 'auto' }}>
                          {it.metrics.views && (
                            ...
                            >
                              {it.metrics.views.toLocaleString()} views
                            </Typography>
                          )}
                          {it.metrics.likes && (
                            <Typography
                            ...
                            >
                              {it.metrics.likes.toLocaleString()} likes
                            </Typography>
                          )}
                          {it.metrics.priceUsd && (
                            <Typography
